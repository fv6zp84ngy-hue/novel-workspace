...
export function makeBlueprint({intent='',route='new',scenario='starter',language='zh',title='',material='',entryMode='natural_language'}={}) {
  ...
  const hasCapturedIntent=route==='new'&&entryMode==='natural_language'&&!!intent.trim();
  ...
  else if(hasCapturedIntent)documents.push({kind:'material',title:english?'Story starting point':'创作起点',content:intent,category:'inspiration'});
  ...
  return {title:name,route,scenario:selected.id,language:english?'en':'zh',documents,tasks:english?['Write and save your first paragraph','Add a reference to this chapter']:['写下第一段并保存','给这一章关联一份资料'],hasImportedMaterial:!!material.trim(),hasCapturedIntent};
}
