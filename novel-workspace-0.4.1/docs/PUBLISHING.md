...
输出为 `dist/novel-workspace-0.4.0.zip`、同名目录与 `SHA256SUMS`。固定清单见 `release-manifest.json`。在干净解压目录重复检查与启动，运行浏览器验收。
...
git init -b main
git add -- .
git commit -m "Release Novel Workspace 0.4.0"
```
...
0.4.0 的模型和 WebDAV 功能需要运行本机 Python 网关。GitHub Pages 只托管基础界面；不要公开监听网关或放入任何默认 key。首次使用事件只在本地浏览器保存，测试导出 JSON 由操作者自行保管。真实作者、账号和设备验收未通过前，请将发布说明标为个人试用版本。

0.4.0 的浏览器协作模块已随发布包附带；从源码重新构建需 Node、`npm ci`、`npm run build:crdt`。发布包无需安装依赖。第三方许可见 `THIRD_PARTY_LICENSES.md`。
